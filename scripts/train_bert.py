...
def main():
    ...
    if not os.path.exists(train_csv):
        print(f"[WARN] Emotion NLP dataset not found at {train_csv}")
        ...
        def dummy_loader():
            ids    = torch.randint(0, 30000, (64, max_length))
            mask   = torch.ones(64, max_length, dtype=torch.long)
            labels = torch.randint(0, 7, (64,))
            from torch.utils.data import TensorDataset, DataLoader
            return DataLoader(
                TensorDataset(ids, mask, labels), batch_size=batch_size
            )
        ...
        class DummyLoader:
            def __iter__(self):
                for _ in range(5):
                    ids    = torch.randint(0, 30000, (batch_size, max_length))
                    mask   = torch.ones(batch_size, max_length, dtype=torch.long)
                    labels = torch.randint(0, 7, (batch_size,))
                    yield {"input_ids": ids, "attention_mask": mask, "label": labels}
    ...
    print(f"[BERT Train] Starting training for {epochs} epochs …\n")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if vl_acc > best_val_acc:
            best_val_acc = vl_acc
            torch.save(model.state_dict(), out_dir / "best_bert.pt")
            print(f"  ✓ Best model saved (val_acc={best_val_acc:.4f})")

    # ── Final evaluation ──────────────────────────────────────────────────────
    print("\n[BERT Eval] Loading best checkpoint …")
    model.load_state_dict(torch.load(out_dir / "best_bert.pt", map_location=device))
    ...
    return str(out_dir / "best_bert.pt")
