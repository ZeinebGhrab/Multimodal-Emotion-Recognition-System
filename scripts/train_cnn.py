...
def main():
    ...
    print(f"[CNN Train] Starting training for {epochs} epochs …\n")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if vl_acc > best_val_acc:
            best_val_acc = vl_acc
            ckpt = out_dir / "best_cnn.pt"
            torch.save(model.state_dict(), ckpt)
            print(f"  ✓ Best model saved (val_acc={best_val_acc:.4f})")

    # ── Final test evaluation ─────────────────────────────────────────────────
    print("\n[CNN Eval] Loading best checkpoint …")
    model.load_state_dict(torch.load(out_dir / "best_cnn.pt", map_location=device))
    ...
    return str(out_dir / "best_cnn.pt")
